...
from mytools import get_dir
import os.path

def fit_poly(x,y,k=2):
    '''参数x和y是二维数据点的两个分量部分，都是一维数值序列，k表示多项式最高次数，默认取值2，表示二项式拟合，函数返回多项式函数以及系数列表，原理参见 学无止境/多项式曲线拟合.md，另外这个返回的多项式函数可以同时计算多个数值，如果你以列表传入的话'''
    n=len(x)
    if n!=len(y):
...
cwd=get_dir()
xy=np.load(os.path.join(cwd,'xy.npz'))
x=xy['x']
y=xy['y']
